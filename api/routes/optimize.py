...
def generate_daily(req: GenerateDailyRequest):
    ...
    # 2. Cargar datos necesarios (params, catalogo, operarios, restricciones)
    params = _load_params()
    catalogo = _load_catalogo()
    operarios = _load_operarios()

    # Cargar restricciones (semana del resultado)
    semana = base_name
    restricciones = _load_restricciones(semana)
    avance_data = _load_avance(semana) if semana else {}

    # 3. Cruzar pedido con catalogo
    # Reconstruir pedido: normalize field names (frontend uses modelo_num, API expects modelo)
    if pedido_snapshot:
        pedido_items = []
        ...
        # Fallback: reconstruct from weekly_schedule entries
        pedido_items = [
    ...
    # 4. Compilar restricciones (sin reopt)
    compiled = compile_constraints(
        restricciones, avance_data, matched, params["days"],
    )

    # 5. Ajustar modelos: usar volumenes del weekly_schedule (no del pedido)
    #    El weekly_schedule YA tiene los pares que el usuario decidio
    models_for_opt = deepcopy(matched)
    weekly_by_model = {}
    ...
    for m in models_for_opt:
        codigo = m["codigo"]
        if codigo in weekly_by_model:
            m["total_producir"] = weekly_by_model[codigo]
        else:
            m["total_producir"] = 0
    ...
    # 9. Aplanar resultados (mismo proceso que optimize)
    model_lookup = {m["codigo"]: m for m in models_for_opt}
    daily_results = {}
    for day_name, day_data in raw_daily.items():
        summary = day_data.get("summary", {})
